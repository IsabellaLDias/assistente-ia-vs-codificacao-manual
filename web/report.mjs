...
export function createTrialPdf(trial) {
  const document = new PDFDocument({size:'A4', margin:54, info:{Title:`LAB02 - ${plain(trial.kata)}`, Author:'LAB02'}});
  ...
  document.fillColor('#64748b').fontSize(8).text(`Gerado em ${new Date().toLocaleString('pt-BR')} - registro ${trial.id}`, 54, 766, {align:'center', width:487, lineBreak:false});
